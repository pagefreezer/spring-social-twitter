...
public class TwitterMappingJackson2HttpMessageConverter<T> extends MappingJackson2HttpMessageConverter {
     ...
     */
    public TwitterMappingJackson2HttpMessageConverter() {
       super();
    }
    ...
    private Object readJavaType(JavaType javaType, HttpInputMessage inputMessage) {
        try {
            checkRateLimit(inputMessage.getHeaders());
    ...
    private void checkRateLimit(HttpHeaders headers) throws ThresholdLimitReachedException {
        if (headers.containsKey("X-Rate-Limit-Remaining")) {
            String remainingRequests = headers.getFirst("X-Rate-Limit-Remaining");
            if (remainingRequests == null || remainingRequests.trim().length() <= 0)
                return;
